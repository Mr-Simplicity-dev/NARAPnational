...
<head>
  ...
  ">
  <title>NARAP Member Dashboard</title>
  ...
  <style>
    :root {
      ...
      --gradient: linear-gradient(135deg, var(--brand-600) 0%, var(--brand-700) 50%, var(--brand-800) 100%);
      --ok: #10b981; 
      --warn: #f59e0b; 
      --err: #ef4444;
    }
...
.welcome-content {
  position: relative;
  z-index: 1;
  text-align: center; /* This centers only the text */
...
.welcome-stats {
  ...
  grid-template-columns: repeat(auto-fit, minmax(200px, 1fr));
  gap: 24px;
  text-align: left; /* Keep stats text aligned left */
}

/* Account Settings Toggle Styles */
#member-settings {
  display: none; /* Hidden by default */
  animation: fadeIn 0.3s ease-in-out;
}

#member-settings.active {
  display: block;
}
...
/* Settings Button Dropdown - Current Position */
.menu {
  position: relative;
}
...
<body>
    ...
    <header class="dashboard-header">
      <div class="header-content">
        <div class="header-brand">
          <img src="/uploads/slider/Narap.png" alt="NARAP" class="header-logo">
          <h1 class="header-title">NARAP Dashboard</h1>
        </div>
        
        <div class="header-user">
          <img src="/uploads/slider/Narap.png" alt="User" class="user-avatar" id="headerAvatar">
          <div class="user-info">
            <h4 id="headerName">Welcome</h4>
    ...
    <main class="dashboard-main">
      ...
      <section class="welcome-hero">
        <div class="welcome-content">
          <h2 class="welcome-title" id="hello">Welcome to NARAP</h2>
          <p class="welcome-subtitle">Manage your membership, payments, and profile all in one place.</p>
          
          <div class="welcome-stats">
            <div class="stat-card">
              <div class="stat-icon">👤</div>
              <div class="stat-value" id="membershipStatus">Active</div>
              <div class="stat-label">Membership Status</div>
            </div>
            <div class="stat-card">
              <div class="stat-icon">💳</div>
              <div class="stat-value" id="paymentStatus">Unpaid</div>
              <div class="stat-label">Payment Status</div>
            </div>
            <div class="stat-card">
              <div class="stat-icon">📅</div>
              <div class="stat-value" id="joinDate">2024</div>
  ...
  <div style="text-align: center; margin-bottom: 32px;">
    <div class="menu" id="mdMenu" style="display: inline-block; position: relative;">
      <button class="settings-toggle-btn" id="mdMenuToggle" aria-haspopup="true" aria-expanded="false">
        <i class="fas fa-cog"></i>
        Account Settings ▾
      </button>
      <div class="menu-list" role="menu" aria-labelledby="mdMenuToggle" style="left: 50%; transform: translateX(-50%);">
        <a href="#" role="menuitem" id="linkEditProfile">
          <i class="fas fa-user"></i> Edit Profile
        </a>
        <a href="#" role="menuitem" id="linkChangePassword">
          <i class="fas fa-lock"></i> Change Password
        </a>
        <a href="#" role="menuitem" id="linkShowAll">
          <i class="fas fa-eye"></i> Show All Settings
        </a>
        <a href="#" role="menuitem" id="linkHide">
          <i class="fas fa-eye-slash"></i> Hide Settings
        </a>
      </div>
    </div>
  </div>
      ...
      <div class="dashboard-grid">
        ...
        <div class="dashboard-card">
          ...
          <div class="profile-section">
            <img src="/uploads/slider/Narap.png" alt="Profile" class="profile-avatar" id="avatar" data-fallback="/uploads/slider/Narap.png">
          ...
          <div style="font-weight: 700; margin-bottom: 16px; color: var(--brand);">Member Details</div>
          <div class="list" id="recentList">
            <div style="color: var(--muted);">Loading member details...</div>
          </div>
        </div>
        </div>
...
<div class="dashboard-card">
  ...
  <!-- Payment Options -->
  <div style="margin-top: 24px;">
    <div style="background: var(--gradient); color: white; padding: 24px; border-radius: 16px; margin-bottom: 16px;">
      <h4 style="margin-bottom: 8px;">Membership Fee</h4>
      <p style="opacity: 0.9; margin-bottom: 16px;">Access member benefits and dashboard tools.</p>
      <div style="font-size: 2rem; font-weight: 800;">₦5,000</div>
      <button class="btn-modern" id="payMembership" data-amount-kobo="500000" data-purpose="membership">
    ...
    <div style="background: var(--gradient); color: white; padding: 24px; border-radius: 16px; margin-bottom: 16px;">
      <h4 style="margin-bottom: 8px;">ID Card Fee</h4>
      <p style="opacity: 0.9; margin-bottom: 16px;">Get your official member ID card.</p>
      <div style="font-size: 2rem; font-weight: 800;">₦3,000</div>
      <button class="btn-modern" id="payIdCard" data-amount-kobo="300000" data-purpose="idcard">
        <i class="fas fa-credit-card"></i>
    ...
    <div style="background: var(--gradient); color: white; padding: 24px; border-radius: 16px; margin-bottom: 16px;">
      <h4 style="margin-bottom: 8px;">Certificate Fee</h4>
      <p style="opacity: 0.9; margin-bottom: 16px;">Generate and download membership certificate.</p>
      <div style="font-size: 2rem; font-weight: 800;">₦10,000</div>
      <button class="btn-modern" id="payCertificate" data-amount-kobo="1000000" data-purpose="certificate">
        <i class="fas fa-credit-card"></i>
...
async function paymentsInit(amount, purpose) {
  const token = getToken();
  const response = await fetch(`${API_BASE}/payments/init`, {
    method: 'POST',
    headers: {
...
function updateDashboard(user) {
  ...
  // Update payment status in welcome stats
  const paymentStatusEl = document.getElementById('paymentStatus');
  if (paymentStatusEl) {
    paymentStatusEl.textContent = user.hasPaidMembership ? 'Paid' : 'Unpaid';
  }
  
  // Update membership status in welcome stats
  const membershipStatusEl = document.getElementById('membershipStatus');
  if (membershipStatusEl) {
    membershipStatusEl.textContent = user.hasPaidMembership ? 'Active' : 'Pending';
  }
...
document.addEventListener('DOMContentLoaded', function() {
  const settingsSection = document.getElementById('member-settings');
